...
def SmoName(d, value):
  return d.encodedUtf16ToUtf8(d.encodeString(value["m_fullname"], 48))
#svDebugInfo("str:%s" % d.encodedUtf16ToUtf8(d.encodeString(value["m_fullname"], 48)))
#value["Record"]["RecordName"].string() "UTF-8"

def QStringValue(d, value):
  return d.encodedUtf16ToUtf8(d.encodeString(value, 48))
...
#    retv += " ("+toHexString(value["m_key"])+") "
#    retv += 'flags:0x%x' % value["m_flags"]
#    d.putValue(retv)

#    smopath = ""
#    parentSmo = value["m_parentSmo"]["wp"]["value"].cast("SMO*")
#    while parentSmo.integer() > 0:
#      smopath = SmoName(d, parentSmo) + " / " + smopath
#      parentSmo = parentSmo["m_parentSmo"]["wp"]["value"].cast("SMO*")

#    devinfo = value["m_deviceInfo"]["wp"]["value"].cast("DeviceInfo*")
...
#  try:
#    d.putValue(SmoName(d,value["m_deviceSmo"]["wp"]["value"].cast("SMO*")))
##    d.putValue(QStringValue(d,value["m_deviceName"]) + (' (addr:0x%x)' % value["m_netAddress"]))
